...
def get_neuron_prominence_2d(img, window_size,fourier_window):
    H, W = img.shape
    # -- Centered gradients
...
def get_neuron_prominence_3d(img, window_size,fourier_window):
        D, H, W = img.shape
        # -- Centered gradients
        ...
        _lambda[:,:,:, 0] = np.real(-1/(3 * a) * (b + C + delta0 / (C+sys.float_info.epsilon)))
        k1 = (-1/2 + 1/2 * np.sqrt(3) * 1)*1j
        _lambda[:,:,:, 1] = np.real(-1 / (3 * a) * (b + k1 * C + delta0 / (C+sys.float_info.epsilon) / k1))
        k2 = (-1 / 2 - 1 / 2 * np.sqrt(3) * 1)*1j
        _lambda[:,:,:, 2] = np.real(-1 / (3 * a) * (b + k2 * C + delta0 / (C+sys.float_info.epsilon) / k2))
...
def cane_2d(org_img, smooth_degree, window_size=15, iter_N=50, kappa1=2, kappa2=2, kappa3=2000):
    H, W = org_img.shape
    ...
    for ite_num in range(iter_N):
        if ite_num % 10 == 0:
            neuron_prominence = get_neuron_prominence_2d(img, window_size,fourier_window)
            weight = 1. / (neuron_prominence+sys.float_info.epsilon)
...
def cane_3d(org_img, smooth_degree, window_size=8, iter_N=30, kappa1=2, kappa2=2, kappa3=2000):
    D, H, W = org_img.shape
    # -- get the optical transfer function of gradient on x and y directions. (similar to psf2otf in Matlab)
    dz, dx, dy = np.zeros((D, H, W)), np.zeros((D, H, W)), np.zeros((D, H, W))
    ...
    for ite_num in range(iter_N):
        if ite_num % 10 == 0:
            neuron_prominence = get_neuron_prominence_3d(img, window_size,fourier_window)
            weight = 1. / (neuron_prominence+sys.float_info.epsilon)
        denorminator2 = 1 + beta * Denormin2
        # -- Step (3) in Alg. 1 in [1].
